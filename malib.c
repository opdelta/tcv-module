...
char** strToArray(char* _line) {
    char* arg = strtok(_line, " ");int size = 0;char ** args = NULL;
    while (arg) {
...
Id* getIdentification(size_t _timestamp, size_t _signature, float _id, unsigned char _emetteurPow) {
    if (_id == 0) _id = 9999;
    if (_emetteurPow == 0) _emetteurPow = 2;
    Id *identification = malloc(sizeof(Id));
    identification->timestamp = _timestamp; identification->signature = _signature; identification->id = _id; identification->emetteurPow = _emetteurPow;
...
TemperatureH* getTemperatureH(size_t _timestamp, size_t _signature, float _temp, int _error) {
    ...
    tempH->timestamp = _timestamp; tempH->signature = _signature;
    if (_error == 0) tempH->temp = _temp;
    return tempH;
}
TemperatureA* getTemperatureA(size_t _timestamp, size_t _signature, float _temp, int _error) {
    ...
    tempA->timestamp = _timestamp; tempA->signature = _signature;
    if (_error == 0) tempA->temp = _temp;
    return tempA;
}
Pulsation* getPulsation(size_t _timestamp, size_t _signature, float _pulse, int _error) {
    ...
    pulseParM->timestamp = _timestamp; pulseParM->signature = _signature;
    if (_error == 0) pulseParM->pulse = _pulse;
   return pulseParM;
}
EchangeDonnees* getEchangeDonnees(size_t _timestamp, size_t _signature, size_t _id, size_t _idpn []) {
    ...
    data->timestamp = _timestamp; data->signature = _signature; data->id = _id;
    return data;
}
RssiSignal* getrssiSignal(size_t _timestamp, size_t _signature, signed short _signalRSSI, size_t _id) {
    ...
    rssi->timestamp = _timestamp; rssi->signature = _signature; rssi->signalRSSI = _signalRSSI; rssi->id = _id;
    return rssi;
}
char* getTransaction(char _line[]) {
   char *signature; signature = calloc(BUFFER_SIZE, sizeof(char));
   if (_line != NULL) {
    signature = strtok(_line, " "); signature = strtok(NULL," ");
   }
   return signature;
}
...
Id* strToId(size_t _lastStamp, char _line[]) {
  char** args = strToArray(_line); size_t timestamp = (size_t)atoi(args[0]); size_t signature = (size_t)atoi(args[1]); float id = atof(args[2]);
  unsigned char pow = (unsigned char)strtol(args[3],NULL,10); Id *ident = getIdentification(timestamp, signature, id, pow);
...
size_t strToRssi(size_t _lastStamp, char _line[], int _pow) {
  signed short signal = -69;char** args = strToArray(_line);size_t timestamp = (size_t)atoi(args[0]);size_t signature = (size_t)atoi(args[1]); size_t id = (size_t)atoi(args[3]); RssiSignal *rssi = getrssiSignal(timestamp, signature, signal, id);
  signal = (signed short)atoi(args[2]); size_t ids = id; float (*pt)(int, int) = &distance; float dist = (*pt)((int)signal, _pow);
  if (timestamp >= _lastStamp) {
...
float strToTemp(size_t _lastStamp, char _line[]) {
  char** args = strToArray(_line);float temp = 0;
  if (strlen(args[2]) > 6) { //Si l'argument 2 est "ERREUR"
    temp = -999;
  } else {
    temp = atof(args[2]);
  }
  if ((size_t)atoi(args[0]) < _lastStamp) temp = -1000;
  free(args);
  return temp;
}
...
unsigned int infoDetail(int _argc, FILE* fp) {
  ...
  printf("\n%d %.1f %.1f %.1f\n%d %zu %zu %zu\n%d %zu %zu %zu", 21, fullTemps[0]/tempCounts[0], fullTemps[1]/tempCounts[1], fullTemps[2]/tempCounts[2],22, errs[0]/3, errs[1]/3, errs[2]/3, 23, errs[3], errs[4], errs[5]);
  free(ident); free(line2);
  return ver;
}